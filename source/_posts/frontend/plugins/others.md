...
![img](https://cdn.beekka.com/blogimg/asset/202607/bg2026070711.webp)
...
### [Maptalks](https://maptalks.org/examples/cn/3d/polygon-altitude/#3d_polygon-altitude)

Maptalks 是一款开源的**WebGIS 开发框架**，基于 JavaScript 开发，专注于提供灵活、可定制的地图可视化能力，适用于 web 端地理信息应用的开发。原生支持**三维地图**（通过 `maptalks.three` 插件），可实现建筑模型、地形可视化等 3D 效果，同时保持二维地图的操作习惯。官方有中文文档，示例详细

**与其他 GIS 框架对比**

| 特性           | Maptalks                   | Leaflet                      | OpenLayers         |
| -------------- | -------------------------- | ---------------------------- | ------------------ |
| 学习曲线       | 中等（自定义性强）         | 低（易上手）                 | 高（功能复杂）     |
| 三维支持       | 原生插件支持               | 需第三方扩展（如 Mapbox GL） | 需第三方扩展       |
| 定制化程度     | 高（从控件到渲染全可定制） | 中（插件生态丰富）           | 高（但配置复杂）   |
| 性能（大数据） | 优秀（聚合图层优化）       | 较好（需插件辅助）           | 优秀（底层优化强） |
...
Lottie 是由 Airbnb 开发的一款**跨平台动画渲染库**，核心作用是将设计工具（如 Adobe After Effects）中制作的复杂动画，以 JSON 格式的 “动画文件”（通常后缀为 `.json`）导出，再通过 Lottie 库在不同平台（Web、iOS、Android、React Native 等）上直接渲染播放，无需开发者手动用代码还原动画细节。

[OpenAnimation](https://github.com/orispok/OpenAnimationApp),这个库收集了很多 lottie 动画

### [autojs6](https://docs.autojs6.com/#/)

autojs 是 基于手机无障碍模式 使用 js 脚本 开发安卓app 自动化流程得工具
...
`register-service-worker`，是一个用于简化 Service Worker 注册过程的工具，提供了常见事件的钩子函数，方便开发者处理 Service Worker 生命周期中的各种状态。

**核心功能**

- 简化 Service Worker 注册流程，支持传入注册选项（如作用域 `scope` 等）。
- 提供多个生命周期钩子函数，包括：
	- `ready`：Service Worker 激活时触发
	- `registered`：Service Worker 注册成功时触发
	- `cached`：内容缓存完成（供离线使用）时触发
	- `updatefound`：发现新内容正在下载时触发
	- `updated`：新内容可用（提示刷新）时触发
	- `offline`：无网络连接时触发
	- `error`：注册过程出错时触发
...
![img](https://cdn.beekka.com/blogimg/asset/202511/bg2025112406.webp)

一个网页 JS 库，用来生成和操作 SVG 图片动画。

### [AntV Infographic](https://infographic.antv.vision/)

![img](https://cdn.beekka.com/blogimg/asset/202512/bg2025122401.webp)

JS 的数据可视化框架，用于在网页生成各种信息图，内置200多种模板。

官方提供提供 AI **Infographic**

​	可以将你在日常写作、汇报或其他文字工作中遇到的内容粘贴到这里，AI 会理解语境并为你生成相匹配的信息图方案

### onlyoffice

ONLYOFFICE 文档是一款完整办公套件,我使用社区免费版在内网测试，只做了预览编辑功能测试，展示效果还行页面是以canvas方式渲染的。word文档页面会有展示异常 pdf不会。编辑功能基本和 office,wps 差不多。编辑后ctrl+s保存功能则需要自己实现这部分逻辑。

下边简介一下 docker-compose部署过程
...
services:
  ...
  onlyoffice-documentserver:
    ...
    restart: always
    # 端口映射：服务器 8080 端口 → 容器 80 端口（测试用8080，避免冲突）
    ports:
      - "8080:80"
    environment:
      # 时区设置（避免日志时间错乱）
      ...
      # 私有IP访问配置（内网必须）
      - ALLOW_INTERNAL_ADDRESSES=true  # ✅ 允许内网IP访问
      - DS_IPADDR_CHECK_DISABLED=true  # ✅ 禁用IP地址检查
      
    # 数据持久化（测试阶段可选，避免容器重启数据丢失）
    volumes:
...
<body>
    ...
    <div class="search-bar">
        <input type="file" id="fileInput" accept=".docx,.doc,.xlsx,.xls,.pptx,.ppt,.pdf" style="margin-right: 10px;">
        <button onclick="uploadFile()">上传并预览</button>
        <input type="text" id="urlInput" placeholder="输入文档URL..." style="margin-left: 20px;">
        <button onclick="previewByUrl()">URL预览</button>
    </div>
    <!-- OnlyOffice编辑器容器 -->
    <div class="docEditor-wrapper">
      <div id="docEditor"></div>
    </div>
    <!-- 引入OnlyOffice官方JS API（替换为你的OnlyOffice地址） -->
    <script type="text/javascript" src="http://192.168.1.244:8080/web-apps/apps/api/documents/api.js"></script>
    <script type="text/javascript">
        // 全局变量：编辑器实例
        let docEditor = null;
        // 本地文件URL（用于预览）
        ...
        function uploadFile() {
            ...
            .then(data => {
                ...
                // 使用后端返回的URL初始化OnlyOffice
                initEditor(data.filename, "http://node-backend:3000/" + data.filepath);
            })
            .catch(error => {
        ...
        // 初始化OnlyOffice编辑器
        function initEditor(docTitle, docUrl) {
            // 检查URL是否为空
            if (!docUrl) {
                return;
            }
            ...
            // 提取文档类型
            const fileExtension = docTitle.split('.').pop().toLowerCase();
            ...
            const docEditorConfig = {
                ...
                document: {
                    ...
                    // 文档类型（text, spreadsheet, presentation, pdf）
                    type: documentType,
                    permissions: {
                        edit: false, // 内网预览仅查看，关闭编辑权限
                        download: true,
                        print: true,
                        comment: false,
                        copy: true
                ...
                editorConfig: {
                    // 语言：中文
                    lang: "zh-CN",
                    // 模式：view=预览（只读），edit=编辑（可写），按需选择
                    mode: "view",
                    // 你的业务系统地址（回调用，可填你的前端地址）
                    callbackUrl: "http://192.168.1.244:3000", // 使用本地地址
                    user: {
                        id: "uid-1769414537402",
                        name: "内网用户"
                    },
                    // 禁用不必要的功能（可选，简化界面）
                    customization: {
                ...
                // 事件回调：编辑器加载完成后触发（关键，搜索高亮需在加载完成后执行）
                events: {
                    onDocumentReady: function() {
                        console.log("OnlyOffice编辑器加载完成！");
                    },
                    onReady: function() {
                        console.log("OnlyOffice编辑器完全就绪！");
                    }
                }
            };
            // 清除容器内容
            document.querySelector(".docEditor-wrapper").innerHTML = "<div id='docEditor'></div>";
            // 创建新的编辑器实例
            docEditor = new DocsAPI.DocEditor("docEditor", docEditorConfig);
        }

        // 页面加载完成后初始化
        window.onload = function() {
            // 显示提示信息
            document.getElementById("docEditor").innerHTML = "<div style='display: flex; justify-content: center; align-items: center; height: 100%; font-size: 18px; color: #666;'>请上传文件或输入URL以预览</div>";
        };
...
app.post('/upload', upload.single('file'), (req, res) => {
  ...
  // 返回文件访问URL（使用服务名，便于Docker Compose网络访问）
...
内网部署后会有一个问题，那就是预览文档时会出现下载失败的问题。这是因为OnlyOffice的安全策略限制，禁止解析私有IP地址。按理说配置环境变量设置
...
就可以的，但是不生效。需要进入onlyoffice运行的容器，打开配置文件
