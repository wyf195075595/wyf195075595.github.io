...
如果不指定 -f 默认在命令当前目录寻找 若找不到`docker-compose.yml`文件， `docker-compose.yml`，则查找 **`docker-compose.yaml`**
...
[代码示例](https://github.com/wyf195075595/docker-project-demo)

本项目 前端使用 vue3+elementPlus+nginx,后端使用 nodejs+express+pm2+mysql 8。下面是整体目录结构
...
services:
  ...
  mysql:
    ...
    environment:
      ...
      MYSQL_DATABASE: ${MYSQL_DATABASE:-docker_demo}
      MYSQL_USER: ${MYSQL_USER:-demo_user}
      MYSQL_PASSWORD: ${MYSQL_PASSWORD:-demo123456}
    ports:
      - "${MYSQL_PORT:-3306}:3306"
    volumes:
      # 将容器内 MySQL 数据目录（/var/lib/mysql）挂载到命名卷 mysql_data，确保容器删除后数据不丢失。
  ...
  backend:
    build:
      context: ./backend
      # dockerfile: 找到名为Dockerfile的文件作为构建上下文
    ...
    environment:
      ...
      DB_HOST: mysql
      DB_PORT: 3306
      DB_USER: ${MYSQL_USER:-demo_user}
      DB_PASSWORD: ${MYSQL_PASSWORD:-demo123456}
      DB_NAME: ${MYSQL_DATABASE:-docker_demo}
      # Root密码用于初始化数据库
      MYSQL_ROOT_PASSWORD: ${MYSQL_ROOT_PASSWORD:-root123456}
    ...
    depends_on:
      mysql:
        # service_healthy 表示等待MySQL服务健康后再启动 是默认值
        # service_started 表示等待MySQL服务启动后再启动
        # service_completed_successfully 依赖的服务是一次性任务（如数据库初始化脚本、数据迁移工具等）
        condition: service_healthy
    networks:
      - app-network
    # 服务启动时会自动初始化数据库（在 index.js 中处理）

  # 前端Vue3服务
  frontend:
    build:
      # 指定构建上下文context和Dockerfile 的路径（它的路径相对于context路径，所以这里不用写./frontend/Dockerfile）
      context: ./frontend
      dockerfile: Dockerfile
      # no_cache: true  # 仅该服务禁用缓存
    ...
    depends_on:
      # 依赖后端服务,等价于 depends_on: backend condition: service_healthy
      - backend
    networks:
      - app-network
...
echo "========================================"
echo "Docker Compose Demo 项目启动脚本"
echo "========================================"
echo ""
...
> `supervisord` 是单容器部署多服务的「必备工具」，核心作用是：
>
> 1. 作为 Docker 容器的唯一主进程，维持容器运行；
> 2. 统一启动、监控、重启容器内的多个子进程；
> 3. 解决 Docker 容器「单进程」的限制，实现多服务共存

目录调整

```
你的项目根目录/
├── Dockerfile          # 上面的单容器Dockerfile
├── supervisord.conf    # 上面的进程配置文件
├── backend/            # 原有后端代码（含package.json、index.js等）
├── frontend/           # 原有前端代码（含package.json、nginx.conf等）
│   └── nginx.conf      # 原有前端Nginx配置文件
└── config/
    └── mysql/
        └── my.cnf      # 原有MySQL配置文件
```
...
```dockerfile
# 基础镜像：基于MySQL 8.0（兼容安装其他依赖）
FROM mysql:8.0

# 设置工作目录
WORKDIR /app

# 1. 配置国内源 + 安装依赖（Node.js/Nginx/Supervisord）
...
# 2. 拷贝MySQL配置（复用你原有的config/mysql/my.cnf）
...
# 5. 配置Supervisord（核心：管理MySQL/Node/Nginx进程）
COPY ./supervisord.conf /etc/supervisor/conf.d/supervisord.conf

# 6. 暴露端口（MySQL 3306 + 后端 3000 + 前端 80）
EXPOSE 3306 3000 80

# 7. 设置MySQL环境变量（复用你原有的默认值）
...
# 8. 启动Supervisord（统一管理所有进程）
CMD ["/usr/bin/supervisord", "-c", "/etc/supervisor/conf.d/supervisord.conf"]
```

supervisord.conf(根目录)

```ini
[supervisord]
nodaemon=true  # 前台运行（Docker必需）
logfile=/var/log/supervisord.log
pidfile=/var/run/supervisord.pid

# 1. 管理MySQL进程
[program:mysql]
command=/entrypoint.sh mysqld  # MySQL默认启动脚本
autostart=true
autorestart=true
stdout_logfile=/var/log/mysql.log
stderr_logfile=/var/log/mysql.err.log
user=root
priority=10  # 优先级最高，先启动

# 2. 管理Node后端进程
[program:backend]
command=npm start
directory=/app/backend  # 后端代码目录
autostart=true
autorestart=true
stdout_logfile=/var/log/backend.log
stderr_logfile=/var/log/backend.err.log
user=root
priority=20  # 等MySQL启动后再启动
startsecs=10  # 延迟10秒启动，确保MySQL就绪
...
```bash
# 构建镜像（命名为 docker-demo:single）
docker build -t docker-demo:single .

# 启动容器（挂载MySQL数据卷，持久化数据）
docker run -d \
  --name docker-demo-single \
  -p 3306:3306 \
  -p 3000:3000 \
  -p 80:80 \
  -v mysql_data_single:/var/lib/mysql \  # 持久化MySQL数据
  -v $(pwd)/backend/uploads:/app/backend/uploads \  # 持久化上传文件
  -v $(pwd)/backend/logs:/app/backend/logs \        # 持久化后端日志
  docker-demo:single
```
...
# ===================== 配置项（可根据需要修改） =====================
IMAGE_NAME="docker-demo:single"  # 镜像名称
CONTAINER_NAME="docker-demo-single"  # 容器名称
MYSQL_VOLUME="mysql_data_single"  # MySQL数据卷名称
# 暴露的端口（和Dockerfile保持一致）
PORTS="3306:3306 3000:3000 80:80"

# ===================== 脚本核心逻辑 =====================
echo -e "\033[32m===== 开始部署单容器版 Docker Demo =====\033[0m"
...
if ! docker volume ls | grep -q "$MYSQL_VOLUME"; then
    echo "   创建数据卷: $MYSQL_VOLUME"
    docker volume create "$MYSQL_VOLUME" > /dev/null
else
    echo "   数据卷 $MYSQL_VOLUME 已存在，跳过创建"
fi

# 3. 构建镜像（强制重新构建，避免缓存问题）
echo -e "\n3. 开始构建镜像: $IMAGE_NAME..."
docker build -t "$IMAGE_NAME" --no-cache .

# 4. 启动容器（挂载必要的卷）
echo -e "\n4. 启动容器: $CONTAINER_NAME..."
docker run -d \
  --name "$CONTAINER_NAME" \
  -p $PORTS \
  ...
  # 如需挂载前端配置文件，取消下面注释
  -v "$(pwd)/frontend/config:/usr/share/nginx/html/config:ro" \
  "$IMAGE_NAME"
...
if docker ps | grep -q "$CONTAINER_NAME"; then
    ...
    echo -e "\n   查看日志命令: docker logs -f $CONTAINER_NAME"
    echo -e "   进入容器命令: docker exec -it $CONTAINER_NAME /bin/bash"
else
