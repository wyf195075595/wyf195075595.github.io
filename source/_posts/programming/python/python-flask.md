...
| 框架    | 定位                             | 优势                                       | 劣势                               | 适用场景                       |
| ------- | -------------------------------- | ------------------------------------------ | ---------------------------------- | ------------------------------ |
| Django  | 全栈框架（ batteries-included ） | 功能全面、安全稳定、生态丰富、后台管理强大 | 略厚重、同步性能一般               | 企业级应用、后台系统、API 服务 |
| Flask   | 微框架（轻量灵活）               | 体积小、可定制化强                         | 需手动拼接第三方库（如 ORM、表单） | 小应用、原型开发、API 原型     |
| FastAPI | 异步 API 框架                    | 异步性能优、自动生成 API 文档              | 无内置后台管理、生态较新           | 高并发 API 服务、实时数据接口  |